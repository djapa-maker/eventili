 ...
 */
package gui.backOffice.reclamations.Consulter;
...
public class ConsulterController implements Initializable {
    ...
    @FXML
    private Button CloturerBoutton;
    @FXML
    ...
    private ReponseService repService = new ReponseService();
    private ReclamationService reclamService = new ReclamationService();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        System.out.println(reclam.getDescription());
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                editMode = false;
                autoupdate = repService.getAllAnswers(reclam).size();
                LoadInfoReclam();
                checkCloturer();
                LoadMessages();
    ...
    private void checkCloturer() {
        if (reclam.getStatus().equals("cloturer")) {
            ...
            CloturerBoutton.setVisible(false);
    ...
    private void LoadInfoReclam() {
        ...
        infoReclam.setText(reclam.getP().getNom_pers() + " " + reclam.getP().getPrenom_pers());
        FontAwesomeIconView fa = new FontAwesomeIconView(FontAwesomeIcon.SEND);
        fa.getStyleClass().add("EnvoyerIcone");
        EnvoyerButton.setGraphic(fa);
        MainView.hoverProperty().addListener(l -> {
            if (autoupdate != repService.getAllAnswers(reclam).size() && !editMode) {
                LoadMessages();
            }
        });
    }
    ...
    @FXML
    private void CloturerBouttonFunction(ActionEvent event) {
        reclam.setStatus("cloturer");
    ...
    private void EnvoyerButtonFunction(ActionEvent event) {
        ...
        if (ControlSaisie(Message)) {
            reclam.setStatus("EnAttenteUser");
            reclamService.modifier(reclam);
    ...
    private Group editMessage(reponse rep) {
        ...
        save.setOnAction(value -> {
            if (ControlSaisie(message.getText().trim())) {
                Alert ConfirmEdit = new Alert(AlertType.INFORMATION);
    ...
    private void LoadMessages() {
        ...
        List<reponse> reps = repService.getAllAnswers(reclam);
        Messages.add(FirstMessage(), 1, 0);
        int rows = 1;
